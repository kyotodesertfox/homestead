...
contract TokenEscrow is UUPSUpgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    // Either party confirms. Release fires automatically when both confirm.
    ...
    // Cancel an escrow.
    // Before funded: initiator cancels unilaterally — no tokens were deposited, nothing to return.
    // After funded: both parties must confirm cancel — ETH returned to counterparty.
    function cancel(uint256 escrowId) external nonReentrant {
        ...
        // After funded: both parties must agree. ETH returned to counterparty, no tokens to return.
